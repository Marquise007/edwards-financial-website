// Cloudflare Pages Function: /api/intake-submit
// Receives the completed client intake from /resources/client-intake (filled PDF,
// or a JSON answer dump when pdf-lib can't fill the template) and emails it to the
// practice via Resend.
//
...
const APP_HEADER = 'client-intake';                 // X-EFA-App value sent by client-intake.html
const MAX_BODY_BYTES = 8 * 1024 * 1024;             // a filled intake runs well under 1 MB
...
export async function onRequestPost(context) {
  ...
  const fileBase = (String(body.filename || 'EFA_Intake').replace(/[^A-Za-z0-9._-]/g, '_')).slice(0, 120)
    .replace(/\.(pdf|json)$/i, '') || 'EFA_Intake';
  const attachments = [];
  ...
  const rows = [
    ...
    ['Application fields captured', meta.fields || 0],
  ...
  const payload = {
    ...
    subject: `New client intake: ${who}`,
    html: wrap(`
      <p style="margin:0 0 14px;">A client intake questionnaire was just submitted from the website.</p>
      ...
      ${outHtml}
      <p style="margin:18px 0 0;color:#6b6b6b;font-size:13px;">The questionnaire is attached${isJson ? ' as a JSON answer file — the PDF template could not be filled in the client\'s browser' : ''}. Review it before any application is submitted.</p>`),
    attachments,
  };
